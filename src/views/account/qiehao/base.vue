  ...
  <div class="qiehao">
    ...
    <div class="qiehao-operation">
      ...
      <div class="qiehao-operation-right">
        ...
        size="mini" 
        @click="addData"
        v-if="$tool.isPerm('account:qiehao:add')"
        > 添加
        </el-button>
      ...
      >
        ...
        >
          <template slot-scope="scope">
            ...
            v-if="$tool.isPerm('account:qiehao:delete')"
            >删除
            </el-button>
          </template>
        </el-table-column>
      ...
      >
        <el-form size="mini" :model="formData" :rules="rules" ref="formData" label-suffix="：" label-width="92px">
          <el-form-item label="分类" prop="category">
            <el-select v-model="formData.category" placeholder="请选择 或 创建分类" allow-create filterable clearable>
              <el-option
              v-for="item in categoryOptions"
              :key="item.value"
              :label="item.label"
          ...
          <el-form-item label="账号" prop="account">
            <el-input size="mini" placeholder="请输入账号" v-model="formData.account" :disabled="dialogTitle == '编辑企鹅号'"></el-input>
          </el-form-item>
...
export default {
  data(){
    return {
      ...
      total: 0,
      rules: {
        category: [{ required: true, message: '请选择或创建分类', trigger: ['blur', 'change'] }],
  ...
  methods:{
    ...
    async getData(){
      ...
      rsp.data.data.categories.forEach((data, index) => {// 过滤分类
    ...
    async downloadExcel(){
      ...
      const header = ['ID', '分类', '昵称', '账号', '密码', '手机号', '账号状态', '运营状态', '描述', '创建日期', '更新日期']
      this.$csv.downloadCsv(dataList, { header }, "企鹅号-基础账号.csv")
    },
    // 添加数据
    async addData(){
        this.resetForm()
    ...
    async deleteData(row){
        ...
        if (result){
          const rsp = await this.$api.deleteQiEHao(row.id)
          if (!rsp) return
    ...
    submitForm(){
      this.$refs.formData.validate(async (valid) => {
        if (valid){
          if (this.dialogTitle == '添加企鹅号'){
            const rsp = await this.$api.addQiEHao(this.formData)
            if (!rsp) return
          ...
          }else if(this.dialogTitle == '编辑企鹅号'){
            const rsp = await this.$api.editQiEHao('base', this.formData)
