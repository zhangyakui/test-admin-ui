  ...
  methods:{
    ...
    async downloadExcel(){
      ...
      rsp.data.data.list.forEach(data => {
        ...
        if (userInfo){
          ...
          user.readCount = userInfo.readCount
      ...
      let header = ['分类', '昵称', '账号', 'UID', '等级', '粉丝', '作品', '总播放', '总评论', '主页链接', '更新日期', '平台镜像', '镜像昵称', '镜像UID', '镜像粉丝', '镜像作品', '起始日期', '截止日期', '镜像主页链接']
      this.$csv.downloadCsv(userList, { header }, "知乎-镜像映射信息.csv")
    ...
    submitForm(){
      this.$refs.formData.validate(async (valid) => {
        if (valid){
          const rsp = await this.$api.editZhiHu('image', this.formData)
