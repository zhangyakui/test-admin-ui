  ...
  <div class="zhihu">
    ...
    <div class="zhihu-operation">
      ...
      <div class="zhihu-operation-right">
        ...
        size="mini" 
        @click="addData"
        v-if="$tool.isPerm('account:zhihu:add')"
        > 添加
        </el-button>
      ...
      >
        ...
        >
          <template slot-scope="scope">
            ...
            v-if="$tool.isPerm('account:zhihu:delete')"
            >删除
            </el-button>
          </template>
        </el-table-column>
      ...
      >
        <el-form size="mini" :model="formData" :rules="rules" ref="formData" label-suffix="：" label-width="92px">
          ...
          <el-form-item label="账号" prop="account">
            <el-input size="mini" placeholder="请输入账号" v-model="formData.account" :disabled="dialogTitle == '编辑知乎'"></el-input>
          </el-form-item>
...
export default {
  ...
  methods:{
    ...
    async downloadExcel(){
      ...
      this.$csv.downloadCsv(dataList, { header }, "知乎-基础账号.csv")
    },
    // 添加数据
    async addData(){
        this.resetForm()
    ...
    async deleteData(row){
        ...
        if (result){
          const rsp = await this.$api.deleteZhiHu(row.id)
          if (!rsp) return
    ...
    submitForm(){
      this.$refs.formData.validate(async (valid) => {
        if (valid){
          if (this.dialogTitle == '添加知乎'){
            const rsp = await this.$api.addZhiHu(this.formData)
            if (!rsp) return
          ...
          }else if(this.dialogTitle == '编辑知乎'){
            const rsp = await this.$api.editZhiHu('base', this.formData)
